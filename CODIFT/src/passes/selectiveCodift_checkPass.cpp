...
public:
    ...
    bool runOnFunction(Function& F) override {
        if (!isSecurityRelevantFunction(F)) return false;
        ...
        for (auto& BB : F) {
            for (auto& I : BB) {
                if (auto* BI = dyn_cast<BranchInst>(&I)) {
                    ...
                    if (calledFunc && isSecuritySink(calledFunc)) WorkList.push_back(&I);
                }
...
private:
    ...
    Value* getSafePtr(Value* V, IRBuilder<>& B, Function* F) {
        if (V->getType()->isPointerTy()) return B.CreatePointerCast(V, Type::getInt8PtrTy(F->getContext()));
