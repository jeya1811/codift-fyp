...
public:
    ...
    bool runOnFunction(Function& F) override {
        ...
        for (Instruction* I : WorkList) {
            if (auto* BO = dyn_cast<BinaryOperator>(I)) injectBinary(BO, &F);
            else if (auto* LI = dyn_cast<LoadInst>(I)) injectLoad(LI, &F);
            else if (auto* SI = dyn_cast<StoreInst>(I)) injectStore(SI, &F);
            else if (auto* CI = dyn_cast<CallInst>(I)) injectTaintSource(CI, &F);
            modified = true;
        }
        return modified;
    }
...
private:
    ...
    Value* getPtr(Value* V, IRBuilder<>& B, Function* F) {
        if (V->getType()->isPointerTy()) return B.CreatePointerCast(V, B.getInt8PtrTy());
