...
struct CheckPass : public FunctionPass {
    ...
    bool runOnFunction(Function &F) override {
        ...
        for(auto *I:targets) inject(sGV,I,M);
        return true;
    }
...
private:
    bool isVerifiedPtr(Value *ptr, GlobalVariable *sGV){
        ...
        if(auto *CE=dyn_cast<ConstantExpr>(ptr)){
            ...
            if(op==Instruction::GetElementPtr){
                if(!tracesTo(CE->getOperand(0),sGV)) return false;
    ...
    void inject(GlobalVariable *gv, Instruction *before, Module *M){
        ...
        FunctionCallee sf=M->getOrInsertFunction("secExcFunc",
            FunctionType::get(Type::getVoidTy(C),{i8,i8p},false));
