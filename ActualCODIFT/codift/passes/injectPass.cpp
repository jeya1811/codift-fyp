  ...
  struct InjectPass : public FunctionPass {
      ...
      bool runOnFunction(Function& F) override {
        ...
        if (isRI) {
          ...
          IRBuilder<> B(first);
          FunctionCallee initFn = M->getOrInsertFunction("codift_init", FunctionType::get(Type::getVoidTy(M->getContext()), {}, false));
          B.CreateCall(initFn, {});
        ...
        //  std::vector<Instruction*> arithWork;
        for (auto& BB : F)
          for (auto& I : BB) {
    ...
    private:
      ...
      bool tagStore(StoreInst* SI, Module* M) {
        ...
        FunctionCallee wf = M->getOrInsertFunction("ramWriteFunc", FunctionType::get(Type::getVoidTy(C), {i8p, i8}, false));
        B.CreateCall(wf, {d, B.getInt8(1)});
        return true;
      }

      /*bool propArith(BinaryOperator* BO, Module* M) {
          ...
          if (!t1 || !t2)
            return false;
          Value* mg = B.CreateOr(t1, t2, "cd.mg");
          Value* rp = resultPtr(BO, B);
          if (!rp)
            return false;
        ...
        Value* resultPtr(Instruction* I, IRBuilder<>& B) {
          LLVMContext& C = B.getContext();
          Type* i8p = PointerType::getUnqual(Type::getInt8Ty(C));
