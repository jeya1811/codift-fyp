...
struct InjectPass : public FunctionPass {
    ...
    bool runOnFunction(Function &F) override {
        ...
        if(!WL.count(F.getName().str())) return false;
        ...
        for(auto &G:M->globals()){
            ...
            if(n=="s"||n.startswith("s.")){sGV=&G;break;}
        }

        std::vector<Instruction*> work;
        for(auto &BB:F) for(auto &I:BB){
            if(auto *SI=dyn_cast<StoreInst>(&I)){
                if(isa<Constant>(SI->getValueOperand())) continue;
                if(isRI){
        ...
        bool changed=isRI; int sc=0,xc=0;
        for(auto *I:work){
            if(auto *SI=dyn_cast<StoreInst>(I))
                {if(tagStore(SI,M)){changed=true;sc++;}}
            else if(auto *BO=dyn_cast<BinaryOperator>(I))
                {if(propXor(BO,M)){changed=true;xc++;}}
        }
        if(sc||xc) errs()<<"[Inject]  "<<sc<<" stores "<<xc<<" xors\n";
        return changed;
    }
...
private:
    bool tracesTo(Value *v,GlobalVariable *gv){
        if(!v) return false;
        if(auto *G=dyn_cast<GlobalVariable>(v)) return G==gv;
        if(auto *G=dyn_cast<GetElementPtrInst>(v)) return tracesTo(G->getPointerOperand(),gv);
        if(auto *B=dyn_cast<BitCastInst>(v))        return tracesTo(B->getOperand(0),gv);
    ...
    bool tagStore(StoreInst *SI,Module *M){
        Instruction *nx=SI->getNextNode();if(!nx)return false;
        IRBuilder<> B(nx);LLVMContext &C=M->getContext();
    ...
    bool propXor(BinaryOperator *BO,Module *M){
        Instruction *nx=BO->getNextNode();if(!nx)return false;
        if(isa<Constant>(BO->getOperand(0))&&isa<Constant>(BO->getOperand(1)))return false;
        IRBuilder<> B(nx);LLVMContext &C=M->getContext();
        Type *i8=Type::getInt8Ty(C),*i8p=PointerType::getUnqual(i8);
        ...
        Value *rp=resPtr(BO,B);if(!rp)return false;
        B.CreateCall(wf,{rp,mg});return true;
    }
    Value *getTag(Value *V,IRBuilder<>&B,FunctionCallee rf){
        if(!V||isa<Constant>(V))return B.getInt8(0);
        Value *p=valPtr(V,B);if(!p)return B.getInt8(0);
    ...
    Value *valPtr(Value *V,IRBuilder<>&B){
        ...
        if(V->getType()->isPointerTy())return B.CreateBitCast(V,i8p,"cd.vp");
        if(isa<PHINode>(V)||V->getType()->isVoidTy())return nullptr;
        Function *F=nullptr;
        if(auto *I=dyn_cast<Instruction>(V))F=I->getFunction();
        if(!F)return nullptr;
    ...
    Value *resPtr(Instruction *I,IRBuilder<>&B){
        LLVMContext &C=B.getContext();
        Type *i8p=PointerType::getUnqual(Type::getInt8Ty(C));
        if(I->getType()->isVoidTy()||isa<PHINode>(I))return nullptr;
        Function *F=I->getFunction();if(!F)return nullptr;
        IRBuilder<> E(&*F->getEntryBlock().getFirstInsertionPt());
